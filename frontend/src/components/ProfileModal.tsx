  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/auth/user/`, {
        method: 'PATCH', // Important: PATCH for partial updates
        headers: {
      ...
      if (response.ok) {
        await response.json();
        // Update local user context is tricky because login expects credentials, 
        // but we can manually reload or check if auth context has a refresh method.
        // For now, we trick it by calling login if we modified username, but ideally we'd have a 'refreshUser' method.
        // Assuming AuthContext automatically re-fetches on next load, but let's force a reload for now or just notify success.
        
        // A better approach if AuthContext exposes a setUser:
        // setUser(updatedUser);
        
        // Since we don't know if setUser is exposed, let's just alert success and close.
        // Or trigger a page reload specific for this update.
